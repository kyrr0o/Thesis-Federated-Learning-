...
# ==========================
CLIENT_ID    = "client2"
DATA_PATH    = "TREE_LEVEL/client2/BTC30Min.csv"
...
def ensemble_iforest_parallel(X_train, X_test, params, n_models=3):
    """
...
def run_client(round_id=1, random_state=None):
    ...
    print(f"[INFO] Starting client {CLIENT_ID} for round {round_id} with seed={random_state}")
    ...
    models_parallel = train_iforest_in_chunks(
        X_train, BEST_PARAMS, n_chunks=CHUNKS, random_state=random_state
    )
    print(f"[TIME] Intra-model parallel (chunks): {time.time() - t0:.4f} sec")

    # --- INTER-MODEL PARALLEL (ENSEMBLE) ---
    t0 = time.time()
    ensemble_models, score_matrix_test, _ = ensemble_iforest_parallel(
        X_train, X_test, BEST_PARAMS, n_models=N_MODELS
    )
    ...
    # --- LOCAL EVALUATION ---
    t0 = time.time()
    test_scores_agg = np.mean(score_matrix_test, axis=0)
    ...
    # sanitize models before sending to global
    ...
    client_forest_package = {
        ...
        # chunk_models kept empty: used only for local speed-up,
        # not transmitted for privacy + communication reasons.
        "chunk_models": [],
...
if __name__ == "__main__":
    # 1) Training + sending sanitized forest (run this BEFORE aggregation)
    run_client(round_id=2)

    # 2) After tree_aggregator for round 1 has been run,
    #    you can uncomment this to evaluate the GLOBAL model on this client:
    # eval_global_on_client(round_id=1)
